...
$('.form_edit').submit(function(event){
    ...
    var posting = $.post(url,{codeEdit: codigoValue},function(data,status){
            if(status == 'success'){
                ...
                var tipoConta = JSON.parse(escapeSpecialChars(data));
                document.getElementById("codeHidden").value = tipoConta.code;
                document.getElementById("contatipo").value = tipoConta.descricao;
            }
        }
        );
});
