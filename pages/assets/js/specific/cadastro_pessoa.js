$.fn.dataTable.ext.search.push(
    function( settings, data, dataIndex ) {
        ...
        var birthDateArray = data[2].split('-');

        if(birthDateArray[0] > 12){
            var birthDateConverted = birthDateArray[1] + '-' + birthDateArray[0] + '-' + birthDateArray[2];
        }
        else{
            var birthDateConverted = data[2];
        }

        var dateBirth = new Date(birthDateConverted) || 0; // use data for the dateExp column

        if(min.getTime() >= max.getTime()){
            alert('Faixa de dados inválida.');
        }
        
        if ( ( !Date.parse(valueMin) && !Date.parse(valueMax) ) || ( !Date.parse(valueMin) && dateBirth.getTime() <= max.getTime() ) || ( min.getTime() <= dateBirth.getTime()   &&  !Date.parse(valueMax)) 
        || ( min.getTime() <= dateBirth.getTime()   && dateBirth.getTime() <= max.getTime() ))
        {
            return true;
        }
        else{
