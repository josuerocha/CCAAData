$(document).ready(function() {
    //Table 1 with the grades
    $('#table_notas tfoot th').each( function () {
        var title = $(this).text().toLowerCase();
        $(this).html( '<input type="text" style = "width:140px;" placeholder="Pesquisar '+ title +'" />');
    } );

    var tableNotas = $('#table_notas').DataTable( {
        "autoWidth": true,
        dom: 'Brtip',
        buttons: [
            {
    ...
    // Apply the search
    tableNotas.columns().every( function () {
        var that = this;
    ...
    //Table 2 with the observations

    var tableObservacoes = $('#table_observacoes').DataTable( {
        "autoWidth": true,
        dom: 'Brtip',
        buttons: [
            {
...
$('.form_edit').submit(function(event){
    ...
    var posting = $.post(url,{codeEdit: codigoValue},function(data,status){
            if(status == 'success'){
                alert(data);
                var observacao = JSON.parse(escapeSpecialChars(data));
                document.getElementById("codeHidden2").value = observacao.code;
                document.getElementById("codeAluno").value = observacao.codeAluno;
...
function Novo(){
    document.getElementById("codeHidden2").value = '';
    document.getElementById("codeAluno").value = '';
